// [TILES-V1] Port of rlx gtamaplib-vc/ui/map.js (verbatim except tileRoot).
// Tile rendering with parent-level fallback during loads.
//
// World coordinate system (from rlx):
//   MAP_W = MAP_H = 32768 (full map size in world meters)
//   ZERO_X = ZERO_Y = 16384 (origin offset — world (0,0) sits at map center)
//   World y INCREASES going north; map pixel y INCREASES going south (Y inverted).
//
// Tile addressing:
//   /tiles/{z}/{z},{y},{x}.jpg   (z 0-6, y then x in filename)
//   Each tile is 256x256 JPG.
//   Zoom 0 = full map at 1024px; each zoom doubles resolution.
...
// Valid tile (x,y) ranges per zoom level. Outside these ranges → ocean (blank).
const TILE_RANGES = {
  0: [[0, 0], [2, 2]],
  1: [[0, 1], [4, 5]],
  2: [[0, 2], [9, 11]],
...
export const TILES_CONST = { MAP_W, MAP_H, ZERO_X, ZERO_Y, TILE_SIZE, MIN_Z, MAX_Z, TILE_RANGES };

export class GtaTileMap {
  constructor(options = {}) {
    this.tileRoot = options.tileRoot || "/tiles";
    this.cache = new Map();
