...
## ADDENDUM MANUEL 2026-07-07 (survit au regen? NON — regenerer puis re-coller ce bloc)
- tools/cycle.py [CYCLE-V1]: LE cycle standard en une commande (--tag, --harvest, --scan,
  --update-baseline, --commit). Fail-fast + resume condense. Push manuel.
- tools/sync_rlx.py [SYNC-RLX-V1]: archeologie upstream automatisee (ADD/SNAP importes,
  MOVE/RENAME/DEL reports, poses SKIP, SKIP_MARKINGS collisions). Etat: gtamapdata/rlx_sync_state.json.
- tools/audit/collision_scan.py [WAR-SCAN-V2]: detecteur collisions/badpixels, verdicts DUAUX
  (ang>15' ET gap>3m). --apply = plan gate. Apres chaque batch de markings (ou cycle --scan).
- tools/audit/tension_audit.py [TENSION-AUDIT-V1]: structure des deltas rejetes (bloqueurs +
  LM charnieres). ATTENTION: lire en METRES depuis DUAL-METRIC (Metro SE B "bloqueur #1" = 0.046m).
- common.residual_dual / cam_rms_dual [DUAL-METRIC-V1]: la formule duale canonique.
- tools/_archive/patches/import_rlx_keys_0706.py: a RELANCER apres re-ancrage Jason (X)
  (seed 12 Islands, gate 25m) — bloque sur les nouvelles tiles yanis.
- tools/fit_minimal.py: fit ypr(+roll)+fov, xyz verrouille. --list, --apply.
- harvest_run.py (RACINE): pipeline gate triangulables+upgrades (resumable /tmp).
- ci_healthcheck.py: FOSSIL-SCAN dual + ORPHAN-SCAN + baselines arcmin ET metres.
- refine_cam_ypr.py: honore excluded_markings [EXCL-FIX-V1]. triangulate_lm: snap z_constraint.
- UI calib: press-drag+loupe+snap epipolaire; pending (Enter/Esc, fleches 0.1px, rename,
  corbeille); right-click add; clic marker = selection+scroll; PANEL-V3 copy pose.
