...
## ADDENDUM MANUEL 2026-07-06 (survit au regen? NON — regenerer puis re-coller ce bloc)
- tools/audit/collision_scan.py [WAR-SCAN-V1]: detecteur collisions de nom/badpixels a l'echelle.
  Scan read-only; --plan; --apply = exclusions+retri gates (resid<8', delta<15m) + snap z_constraint.
  A rouler apres chaque batch de markings/imports.
- tools/_archive/patches/import_rlx_keys_0706.py: import rlx Keys (idempotent) — a RELANCER apres
  re-ancrage de Jason's Safehouse Vehicles (X) pour seeder les 12 Islands (gate 25m vs xyz rlx).
- tools/fit_minimal.py: fit ypr(+roll via --solve-roll)+fov, xyz verrouille. --list = scan sous-determinees. --apply.
- harvest_run.py (RACINE): pipeline gate triangulables+upgrades (dry, <8', delta<15m, resumable /tmp).
- server /api/fit_minimal?cam&x&y&z[&yaw&pitch&rollv&roll=1]: fit dry sur etat client (reste de PANEL-V2, curl-able).
- /api/delete_pixel: gere provenance (drop source, quarantaine orphelin) [PROVENANCE-V1].
- ci_healthcheck.py: + FOSSIL-SCAN (sources rejettent leur LM >15') + ORPHAN-SCAN (sources listees, aucune ne marque).
- refine_cam_ypr.py: honore excluded_markings.json [EXCL-FIX-V1].
- common.find_fossils(threshold=15): scan programmatique.
- UI calib: marking press-drag+loupe+snap epipolaire; pending (Enter/Esc, fleches 0.1px, rename, corbeille);
  right-click = add sur place; clic marker = selection+scroll liste; adjust (8 sliders, affichage pur);
  PANEL-V3: clic valeur = copie, copy pose.
