...
md.append('   (only once per session — produces tier classifications)\n')
...
md.append('6. If verdict OK: apply via calib UI Optimize, or refine_camera.py\n')
md.append('7. Re-run compute_confidence_tiers.py to update tiers\n')
md.append('8. Run bundle_adjust.py for global refinement\n')
md.append('```\n\n')

md.append('### Calibration order — "What order to work on?"\n')
md.append('```\n')
md.append('python3 tools/calibration_order.py --tier unverified\n')
md.append('python3 tools/calibration_order.py --cams "Cam A,Cam B"\n')
md.append('```\n\n')

md.append('### Outliers detection — "Which pixels are bad?"\n')
md.append('```\n')
md.append('python3 tools/outliers_report.py\n')
md.append('```\n\n')
